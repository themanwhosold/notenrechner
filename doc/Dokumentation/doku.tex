...
Die Web-Oberfläche für die Administartoren wurde mittels JSF Seiten realisiert. Die Seiten sind mit dem JSF-Template templates/adminTemplate.xhtml implementiert. Die Bedienoberfläche hat das Ziel, die Funktionalitäten und Berechtigungen eines Administartors abzubilden und bereitzustellen. Dazu wurden die JSF-Seiten adminSeite.xhtml, studentenVerwalten.xhtml und neuenStudentenAnlegen.xhtml geschrieben. Im Hintergrund der Seiten arbeitet die Managed-Bean mit den Klassennamen AdminManagedBean, die für den Kontrollfluss auf den Seiten verantwortlicht ist. Geplant ist die Hauptfunktionen dieser Java-Bean in die Java-Enterprise Bean AdminSessionBean (bisher nur Stub-Klasse) auszulagern. Insgesamt wurden hier nicht alle geplanten Funktionen umgesetzt bzw. arbeiten nicht fehlerfrei.
